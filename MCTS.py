    ...
    def expand(self, action_probs):
        for action, prob in enumerate(action_probs):
            if(prob > 0):
                ...
                self.children[action] = Node(prior=prob, gamestate=new_gamestate)
...
class MCTS():
    ...
    def __init__(self, game: Game, score_func = UCB, root : Node = None, predict_func = None) -> None:
        ...
        self.predict_func = predict_func if predict_func else partial(dummy_predict, game.move_cardinality)
    ...
    def run_simulation(self):
        ...
        action_probs = self.predict_func(node.gamestate.get_int_list_representation())
        # print(action_probs)
        ...
        for _ in range(M):
            ...
            while reward == None: # i.e. we are not in a terminal state
                action_probs = self.predict_func(self.current_gamestate.get_int_list_representation())
                norm_action_probs = self.normalize_action_probs(action_probs=action_probs, gamestate=self.current_gamestate)
    ...
    def get_norm_action_probs(self) -> np.ndarray:
        ...
        norm_action_probs = self.normalize_action_probs(action_probs, self.initial_gamestate)
        return norm_action_probs
